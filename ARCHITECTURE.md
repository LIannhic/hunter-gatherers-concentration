...
- **Systems** : Mettent à jour l'état du monde via l'**Engine** (`engine.go`).
  ...
  - **ResourceLifecycleSystem** : Gère la maturation des ressources. Supporte les **cycles cycliques** (`Cyclic: true`) : les ressources reviennent au stade 0 après le stade maximum. La propagation a lieu au début du dernier stade (`TurnsInStage == 0`) et le compteur de propagation est réinitialisé à chaque nouveau cycle.
...
- **Renderer**: Dessine le plateau central avec espacement dynamique.
  ...
  - **Cadres d'Action** : Rendu des cadres colorés (`StrokeRect` 3px) via `RenderTileActionFrame()` et `RenderExitTileActionFrame()`. Couleurs : Vert (interactive), Rouge (impossible), Orange (indisponible). Cadre permanent sur la 1ère tuile révélée, au survol pour les autres. Les tuiles de sortie (navigation) utilisent la même logique colorée avec `computeExitTileActionState()` : rouge si navigation scellée, orange si tuile révélée en attente de paire, verte si retournable ou navigable.
- **Input**: Capture les événements (clavier, souris, tactile), gère la navigation entre les zones et les raccourcis clavier. 
  ...
  - **Cadres d'Action** : Système hybride de feedback visuel sur les tuiles et les sorties de navigation. `computeTileActionState()` évalue l'état d'interaction (vert/rouge/orange) en croisant `isProcessing`, `TileState`, `ImmunityTurns`, animations de mouvement et mana. `computeExitTileActionState()` gère les sorties : rouge si navigation scellée, orange si tuile révélée en attente de paire, verte si retournable ou navigable. Rendu via `RenderTileActionFrame()` / `RenderExitTileActionFrame()` dans le renderer (cadre `StrokeRect` 3px). Cadre permanent sur la 1ère tuile révélée, au survol pour les autres.
