...
func NewWorld(width, height int) *World {
	return &World{
		Grid:            board.NewGrid(width, height),
...
// SpawnResource crée une ressource dans le monde
func (w *World) SpawnResource(rtype string, pos entity.Position) (*resource.Resource, error) {
...
// SpawnCreature crée une créature dans le monde
func (w *World) SpawnCreature(species string, pos entity.Position) (*creature.Creature, error) {
...
// RevealTile révèle une tuile
func (w *World) RevealTile(pos board.Position) (*board.Tile, error) {
	tile, err := w.Grid.Reveal(pos)
	if err != nil {
		return nil, err
	}
...
// MatchTiles appaire deux tuiles
func (w *World) MatchTile(pos board.Position) error {
	if err := w.Grid.Match(pos); err != nil {
		return err
	}
	tile, _ := w.Grid.Get(pos)
...
func (s *PropagationSystem) Update(world *World) {
	...
	for _, e := range resources {
		...
		pos := e.GetPosition()
		neighbors := world.Grid.GetNeighbors(board.Position{X: pos.X, Y: pos.Y})
...
func (s *CreatureAISystem) Update(world *World) {
	...
	for _, e := range creatures {
		...
		// Prend une décision
		action := ai.Decide(c, &worldAdapter{world: world})
...
func (wa *worldAdapter) IsValidMove(pos entity.Position) bool {
	tile, err := wa.world.Grid.Get(board.Position{X: pos.X, Y: pos.Y})
	if err != nil {
		return false
	}
...
func (s *TriggerSystem) Update(world *World) {
	// Vérifie les conditions de trigger sur chaque tuile
	for _, tile := range world.Grid.Tiles {
		...
		// Évalue la condition
		if s.checkCondition(trigger.Condition, tile, world) {
			s.executeAction(trigger.Action, tile, world)
			if trigger.Consumed {
				// Supprime le trigger
...
func (s *TriggerSystem) checkCondition(condition string, tile *board.Tile, world *World) bool {
	switch condition {
	...
	case "creature_on_resource":
		...
		neighbors := world.Grid.GetNeighbors(tile.Position)
...
func (s *TriggerSystem) executeAction(action string, tile *board.Tile, world *World) {
	switch action {
	case "creature_flee":
